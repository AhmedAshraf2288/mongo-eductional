...
import { useSingleWatchItemDataLink } from "../../../../../queries/queries";
...
export default function WatchCourseQuizLink({ withTimer, type }) {
  const { token } = useParams();

  const { data: watchCourseQuizData, isLoading: watchCourseQuizDataLoading } =
    useSingleWatchItemDataLink(token);
  ...
  const submitAssignmentMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(["item-link", token], {
        ...watchCourseQuizData,
        is_submitted_before: true,
    });
    navigate(`/profile/exams-results`);
  ...
  function handleQuizSubmission(data) {
    ...
    if (willSubmit) {
      ...
      submitAssignmentMutation.mutate({
        quiz_id: watchCourseQuizData?.id,
        answers: finalAnswer,
  ...
  if (useSingleWatchItemDataLink?.is_time_expired) {
    return (
      <div
      ...
      >
        ...
        <div className="">
          <span className="mb-3 d-block text--red fw-bold">
            تاريخ الامتحان من : {useSingleWatchItemDataLink?.available_from}
          </span>
          <span className="d-block text--red fw-bold">
            الي : {useSingleWatchItemDataLink?.available_to}
          </span>
        </div>
      </div>
    );
